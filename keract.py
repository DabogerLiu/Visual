...
def get_jaco(model, x, cls,layer_name=None):
    """
    Get output activations for all filters for each layer
    :param model: keras compiled model or one of ['vgg16', 'vgg19', 'inception_v3', 'inception_resnet_v2',
    'mobilenet_v2', 'mobilenetv2']
    ...
    firstjaco = _evaluate(model, layer_outputs, x, y=None)
    firstjaco_dict = OrderedDict(zip([output.name for output in layer_outputs], firstjaco))
    firstjaco_inputs_dict = OrderedDict(zip([output.name for output in input_layer_outputs], x))
    result = firstjaco_inputs_dict.copy()
    result.update(firstjaco_dict)
    return result
